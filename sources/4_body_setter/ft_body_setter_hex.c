...
/*   Updated: 2020/11/23 19:36:35 by spark            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_body_setter_hex(t_set *set)
{
	...
	if (((set->flags.shap_flag == 1) && (set->num_ull != 0)))
	{
		if (((set->flags.zeroflag == 1) && ((int)ft_strlen(tmp_body)\
		+ 2 < set->lenths.total_len) && (set->precision_yn == 0)))
			padding++;
		else
			ft_shap_adder(set);
	}
